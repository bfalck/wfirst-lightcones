...
#### Purpose:  Generates parameter setup file for mock survey fields.
...
class Cosmology:
    def __init__(self, H0=70.0, WM=0.27,WV=0.73):
        ...
        self.WM=WM
        self.WV=WV
        self.cosmology_object=FlatLambdaCDM(H0=H0,Om0=WM)
        self.redshift_grid = np.logspace(-3,2,100)
        self.comoving_mpc_grid = self.cosmology_object.comoving_distance(self.redshift_grid).value #np.asarray([(cc.cosmocalc(zf,H0=self.H0,WM=self.WM,WV=self.WV))['DCMR_Mpc'] for zf in self.redshift_grid])
        self.DA_mpc_grid = self.cosmology_object.angular_diameter_distance(self.redshift_grid).value #np.asarray([(cc.cosmocalc(zf,H0=self.H0,WM=self.WM,WV=self.WV))['DA_Mpc'] for zf in self.redshift_grid])
...
if __name__=="__main__":
    ...
    data = ascii.read('snap_v_redshift_Cosmo0.txt')	
	
    zlist = data['col2']
    
	
    namelist = data['col1']
    namelist = np.asarray([(s)[90:93] for s in namelist])
