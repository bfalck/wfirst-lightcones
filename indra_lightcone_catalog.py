...
#import illustris_python as ilpy   #see www.illustris-project.org if needed


ilh = 0.704 # same as indra, so leave it as is
illcos = astropy.cosmology.FlatLambdaCDM(H0=70.4,Om0=0.2726,Ob0=0.0456)


# ======================================================================================================================
#       This code performs the translation and rotation to the camera reference frame and
...
## Column 21:  Radius buffered to subtend FOV [Comoving h^-1 kpc]
    def __init__(self,lightconefile,basedir,mass_limit=(7e10),sfr_limit=0.0,mag_limit=None):
        ...
        self.snapshot_num = lc_data['col2'].data
        ...
        # for basedir = '/home/idies/workspace/indra/X_Y_Z/', indra_id = X_Y_Z
        self.indra_id = basedir[28:33]
    ...
    def process_lightcone(self,minz=0.0,maxz=20.0):
        ...
        for i,cyl in enumerate(self.cylinder_number):
            ...
            snapnum = self.snapshot_num[i]
            print("    Snapshot Number: ", snapnum)
            
            TotNgroups,NTask = indra.getfofheader(2,0,0,snapnum,self.basedir)
            if TotNgroups == 0:
                cylinder_obj = None
                self.cylinder_object_list.append(cylinder_obj)
                continue
            
            subhalos = indra.getsubcat(2,0,0,snapnum,self.basedir)
            
            subhalos['count'] = TotNgroups
            print("    Loaded halo catalog: ",subhalos['count'])

            # Indra subhalo positions are in Mpc/h, below converts to kpc/h to match Ingress and Egress units
...
#            mhalo_msun = subhalos['SubhaloMass']*(1.0e10)/ilh
            mhalo_msun = subhalos['M200crit']/ilh
...
#            gmag=gmag_ABabs+distmod
            ...
            #cull liberally by mass... and number of particles?
            if self.mag_limit is not None:
                print('Ignoring magnitude limit: ', self.mag_limit)
#            mi = np.where(mhalo_msun > self.mass_limit)[0]
            mi = np.where(np.logical_and(mhalo_msun > self.mass_limit, subhalos['subLen'] > 50 ))[0]
...
#            print("    Mag  statistics : ", np.min(gmag[mi]), np.max(gmag[mi]), np.median(gmag[mi]))
            ...
            xpos = subhalos['SubPos'][mi,0] #in kpc/h of max bound part
            ypos = subhalos['SubPos'][mi,1]
            zpos = subhalos['SubPos'][mi,2]
            ...
            camera = Camera([0,0,0],[self.camdir_x,self.camdir_y,self.camdir_z],[self.camup_x,self.camup_y,self.camup_z])
            ...
            velX = subhalos['SubVel'][mi,0]
            velY = subhalos['SubVel'][mi,1]
            velZ = subhalos['SubVel'][mi,2]
...
#            pyplot.show(f1)
    ...
    def periodicize(self,subhalos,boxL):
        # Copies the subhalos dictionary into 7 catalogs: center + 2 before/after in x, y, and z
        # boxL is in kpc/h: convert SubPos to kpc/h from Mpc/h here
        
        # index quantities that exist for every subhalo to those of their first subhalo
        subindex = subhalos['FirstSubOfHalo'][subhalos['M200crit'] > 0]
        fofindex = np.where(subhalos['M200crit'] > 0)[0]
        subhalos['count'] = len(fofindex)
        
        xpos = subhalos['SubPos'][subindex,0].flatten()*1000.
        ypos = subhalos['SubPos'][subindex,1].flatten()*1000.
        zpos = subhalos['SubPos'][subindex,2].flatten()*1000.
        ...
        keys = subhalos.keys()
        subkeys = ['subLen','subOffset','subParentHalo','SubVel','SubVelDisp','SubVmax','SubSpin','SubMostBoundID','SubHalfMass']
        fofkeys = ['NsubPerHalo','FirstSubOfHalo','M200mean','R200mean','M200crit','R200crit','M200tophat','R200tophat']
        ...
        for key in keys:
            if key=='SubPos':
                ...
                new_subhalos[key] = np.concatenate((new_subhalos[key][subindex],subhalos[key][subindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][subindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][subindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][subindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][subindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][subindex])) #7 total boxes
            elif key in fofkeys:
                new_subhalos[key] = np.concatenate((new_subhalos[key][fofindex],subhalos[key][fofindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][fofindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][fofindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][fofindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][fofindex]))
                new_subhalos[key] = np.concatenate((new_subhalos[key],subhalos[key][fofindex])) #7 total boxes
    ...
    def output_catalog(self,outfile):
        ...
        fobj.write('## Halo Mass Lower Limit (Msun) : {:10.5e}'.format(self.mass_limit)+'\n')
        fobj.write('## Assumed Cosmology: '+WMAP7.__str__()+'\n')
        fobj.write('## Created by:  Bridget Falck (JHU) and Greg Snyder (STScI) \n')
        fobj.write('## Based on the Indra simulation %s \n' % self.indra_id)
...
#        fobj.write('## Column 22: [Msun] Total gas mass within 2X stellar half mass radius\n')
        fobj.write('## Column 21: [Msun] Total mass of this subhalo (excludes children subhalos) \n')
...
#        fobj.write('## Column 27: [(10^10 Msun/h) / (0.978 Gyr/h)] Total BH accretion rate within subhalo\n')
        fobj.write('## Column 22: [Mpc] Camera X in Observer Coordinates (Proper X at z; a transverse coordinate) \n')
        fobj.write('## Column 23: [Mpc] Camera Y in Observer Coordinates (Proper Y at z; a transverse coordinate)\n')
        fobj.write('## Column 24: [Mpc] Camera Z in Observer Coordinates (Proper Z at z; should be almost exactly Column 16)\n')
...
#        fobj.write('## Column 34: [AB Mag] Intrinsic stellar z absolute magnitude (BC03) \n')
        fobj.write('## Column 25: [km/s] Galaxy motion in transverse Camera X direction \n')
        fobj.write('## Column 26: [km/s] Galaxy motion in transverse Camera Y direction \n')
        fobj.write('## Column 27: [km/s] Galaxy motion in line-of-sight Camera Z direction ; the Peculiar Velocity \n')
        fobj.write('## Column 28: [km/s] Cosmological expansion velocity at true z (Column 10 measures Column 27+28)\n')
...
#        self.mbh_msun = subhalos['SubhaloMassInRadType'][self.subhalo_index,5]*(1.0e10)/ilh
        self.mhalo_msun = subhalos['M200crit'][self.subhalo_index]/ilh
        
#        self.baryonmass_msun = self.mstar_msun + self.mgas_msun + self.mbh_msun #within 2x stellar half mass radius... best?
        
        self.xpos_ckh = subhalos['SubPos'][self.subhalo_index,0]*1000. #in comoving kpc/h of max bound part
        self.ypos_ckh = subhalos['SubPos'][self.subhalo_index,1]*1000.
        self.zpos_ckh = subhalos['SubPos'][self.subhalo_index,2]*1000.
        ...
        self.xvel_kms = subhalos['SubVel'][self.subhalo_index,0]
        self.yvel_kms = subhalos['SubVel'][self.subhalo_index,1]
        self.zvel_kms = subhalos['SubVel'][self.subhalo_index,2]
    ...
    def print_cylinder(self,outobj):
        ...
        for i,shi in enumerate(self.subhalo_index):
            ...
            thisline = '{:8d}{:12d}  {:12.6f}  {:12.6f}  {:10.2f}  {:10.2f}  {:10.2f}  {:10.2f}  '\
                       ...
                       '{:12.4e}'\
                       '{:10.4f}  {:10.4f}  {:16.4f}  {:8.2f}  {:8.2f}  {:8.2f}  {:12.4e}'\
                      '\n'.format(self.snapshot_number[i],shi,self.RA_deg[i],self.DEC_deg[i],self.RA_kpc[i],self.DEC_kpc[i],self.observed_RA_kpc[i],self.observed_DEC_kpc[i],self.cosmological_redshift[i],self.galaxy_observed_z[i],self.galaxy_peculiar_z[i],self.kpc_per_arcsec[i],self.galaxy_comoving_x_mpc[i],self.galaxy_comoving_y_mpc[i],self.galaxy_comoving_z_mpc[i],self.angdiam_mpc[i],self.observed_angdiam_mpc[i],self.snapz[i],self.center_z[i],self.cylinder_number[i],self.mhalo_msun[i],self.galaxy_camera_posx[i],self.galaxy_camera_posy[i],self.galaxy_camera_posz[i],self.galaxy_camera_velx[i],self.galaxy_camera_vely[i],self.galaxy_camera_velz[i],self.hubble_velocity[i])
